...
public:
    ...
    void queueAsHighestPriority(MacroAct m,bool gasSteal = false);		// queue at the highest priority
    void queueAfterSpore(MacroAct m);                           // queue at highest priority except after spore
    void queueAsLowestPriority(MacroAct m);						// queue at the lowest priority
