...
void CombatCommander::update(const BWAPI::Unitset & combatUnits)
{
    ...
    if (frame8 == 1)
    {
        ...
        updateWatchSquads();
        updateReconSquad();
		//updateClearanceSquad();		// DISABLED, maybe later it can be fixed up
        updateAttackSquads();
    }
    else if (frame8 % 4 == 2)
    {
...
void CombatCommander::chooseScourgeTarget(const Squad & sourgeSquad)
{
    ...
    for (const auto & kv : the.info.getUnitData(the.enemy()).getUnits())
    {
        ...
        if (!ui.type.isFlyer() ||           // excludes all buildings, including lifted buildings
            ...
            the.airHitsFixed.inRange(BWAPI::TilePosition(ui.lastPosition)))
        {
            continue;
        }
...
void CombatCommander::updateWatchSquads()
{
    ...
    for (Base * base : the.bases.getAll())
    {
        if (nWatchers > 0 &&
            ...
            (the.enemyRace() == BWAPI::Races::Terran || the.groundHitsFixed.at(base->getCenterTile()) == 0))
        {
...
// Zerg only.
// Possibly assign one zergling to clear spider mines or non-attacking enemies from our natural.
// TODO This is disabled (not called). It's not in working order.
void CombatCommander::updateClearanceSquad()
{
	return;

	const std::string squadName = "Clear";
	...
	bool assignSquad = false;
	Base * natural = the.bases.myNatural();

	if (natural &&
		natural->getOwner() != the.self() &&
		the.bases.myMain()->getSafeTileDistance(natural->getTilePosition()) >= 0
		/*the.my.completed.count(BWAPI::UnitTypes::Zerg_Zergling >= 4)*/
		)
	{
		// It's safe to move there, so no enemy combat unit is there (except possibly spider mines).

		BWAPI::Broodwar->printf("clearance squad, checking enemy units");
		...
		for (const auto & kv : the.info.getUnitData(the.enemy()).getUnits())
		{
			...
			if ((!ui.goneFromLastPosition || ui.updateFrame >= the.now() - 24) &&		// still there or recently gone
				...
				ui.lastPosition.y <= natural->getPosition().y + 7 * 32 &&
				!ui.type.isFlyer() &&													// zergling can't reach flyers
				!(ui.unit && ui.unit->isFlying()) &&									// including floating buildings
				!ui.type.isSpell() &&
				!ui.type.isInvincible())
			{
				assignSquad = true;
				BWAPI::Broodwar->printf("clearance squad go!");
				break;
			}
		}
	}

	if (!assignSquad && !_squadData.squadExists(squadName))
	{
		return;
	}
	...
	if (assignSquad) BWAPI::Broodwar->printf("clearance squad has a target");

	// The squad is empty and should not be. Try to assign a zergling.
	if (assignSquad && clearanceSquad.isEmpty())
	{
		for (BWAPI::Unit unit : _combatUnits)
		{
			if (unit->getType() == BWAPI::UnitTypes::Zerg_Zergling &&
			...
			{
				BWAPI::Broodwar->printf("clearance squad has a zergling");
				_squadData.assignUnitToSquad(unit, clearanceSquad);
				clearanceSquad.setOrder(SquadOrder(SquadOrderTypes::Attack, natural->getCenter(), 5 * 32, "Clear natural"));
				break;
			}
		}
	}
	...
	// Drop the squad if it is no longer needed. Don't clutter the squad display.
	// It might be empty even if assignSquad is true if all zerglings had higher-priority jobs.
	if (!assignSquad || clearanceSquad.isEmpty())
	{
		_squadData.removeSquad(squadName);
		BWAPI::Broodwar->printf("clearance squad done");
	}
}
        ...
        for (BWAPI::Unit unit : the.enemy()->getUnits())
        {
                ...
                dist < defenseRadius + 384 && zone == the.zone.ptr(unit->getTilePosition()))
            {
                ...
                else if (unit->getType().isWorker())
                {
                    ++nEnemyWorkers;
                    /* CODE ADDED */
                    // Very specific anti-worker-rush cases
                    if (unit->isRepairing() && unit->getTarget() && unit->getTarget()->getType().isWorker()) {
                        ++nEnemyRepairingWorkers;
                    }
                    else {
                        auto target = unit->getTarget();
                        if (target != nullptr && target->getType().isWorker() && target->getPlayer() != BWAPI::Broodwar->enemy()) {
                            nEnemyAttackingWorkers++;
                        }
                    }
                }
...
SquadOrder CombatCommander::getAttackOrder(Squad * squad)
{
    ...
    // TODO Disabled. There is a bug affecting the map Pathfinder.
