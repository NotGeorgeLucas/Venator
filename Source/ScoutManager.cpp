...
void ScoutManager::workerGoHome()
{
	...
	if (!home)
	{
		...
		{
			if (base->getOwner() == the.self() && base->getSafeTileDistance(_workerScout->getTilePosition()) >= 0)
			{
				home = base;
				break;
			}
	...
	if (home)
	{
		if (home->getSafeTileDistance(_workerScout->getTilePosition()) <= 14)
		{
			// We're there.
			releaseWorkerScout();
		}
...
void ScoutManager::moveAirScout()
{
    ...
    if (enemyBase)
    {
        ...
        if (!_overlordAtEnemyBase)
        {
			...
			the.micro.MoveToBase(_overlordScout, enemyBase, enemyBase->getCenter(), true);
    ...
    else
    {
        ...
        {
			the.micro.MoveToBase(_overlordScout, _overlordScoutTarget, _overlordScoutTarget->getCenter(), true);
		}
    }
}
...
bool ScoutManager::releaseScoutEarly(BWAPI::Unit worker) const
{
	...
	if (_overlordScout &&
		...
		the.bases.enemyStart()->getDepot()->getDistance(_overlordScout) <= 14 * 24 &&
